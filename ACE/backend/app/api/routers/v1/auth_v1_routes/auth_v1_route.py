...
@auth_v1_router.get("/mode", response_model=ApiEnvelope[AuthModeResponse])
async def auth_mode(
...
@auth_v1_router.get("/login", name="auth_login")
...
@auth_v1_router.get("/callback", name="oauth_callback")
...
async def me(
    ...
    return ApiEnvelope(data=MeResponse(user=_to_profile(context), csrf_token=None))


@auth_v1_router.post("/logout", name="auth_logout", response_model=ApiEnvelope[dict])
async def logout(
    response: Response,
    ...
    return ApiEnvelope(data={"Logged out": True})