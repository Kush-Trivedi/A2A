...
from a2a.client import A2ACardResolver, AuthInterceptor, ClientConfig, create_client
...
class A2AClientService:
    """ACE's single gateway for talking to remote A2A agents.

    Chat and orchestration code never touch raw protocol objects — they
    consume `A2AStreamEvent`s. Errors always surface as ACE typed errors.
    """
    ...
    def __init__(
        ...
        self._credentials = AceCredentialService(
            token_provider or get_service_token_provider()
        )
    ...
    async def _resolve_card(
        ...
        """Card fetch with TTL cache — avoids a discovery round-trip per turn."""
        cache_key = f"{base_url}{relative_card_path}"
        cached = self._card_cache.get(cache_key)
        if cached is not None and (time.monotonic() - cached[1]) < self._settings.card_cache_ttl_seconds:
            return cached[0]
        resolver = A2ACardResolver(httpx_client, base_url, agent_card_path=relative_card_path)
    ...
    def split_card_url(card_url: str) -> tuple[str, str]:
        """Split a full card URL into (base_url, relative_card_path)."""
    ...
    async def stream_message(
        ...
        *,
        ...
        call_context = AceCredentialService.context_for(auth_audience)
        ...
        try:
            ...
            client = await create_client(
                ...
                interceptors=[AuthInterceptor(self._credentials)],
            )
            async for response in client.send_message(request, context=call_context):
                for event in self._map_response(
                    response, tracker, seen_artifact_ids
                ):
            ...
            if not yielded_text and not tracker.needs_user_action:
                logger.warning(
                    "A2A agent produced no text output",
                    extra=tracker.summary(),
                )
    ...
    def _outcome(
        ...
        # Message-only responses never open a task; a streamed answer with no
        # failure is a completed turn.
