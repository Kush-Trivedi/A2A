...
class FileQaAgent:
    """Session-strict file Q&A.

    Retrieval is scoped to the caller's chat session ONLY (no shared
    knowledge sources) — the agent cannot answer from anything the user did
    not upload in this conversation. No documents -> ask for an upload.
    A real implementation feeds the retrieved chunks to the team's LLM;
    the template quotes the grounded snippets directly.
    """
...
class FileQaAgentExecutor(AgentExecutor):
