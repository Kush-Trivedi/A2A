...
class PlaceholderPolicy:
    ...
    The system never special-cases these to "work anyway" — it reports them
    precisely so swapping in real credentials is a pure yaml change.
    """
...
class SettingsValidator:
    """Validates the resolved configuration at startup: required keys that
    are missing, and any value still carrying the template placeholder.
    Findings name the exact dotted yaml path."""
    ...
    def validate_and_log(self) -> SettingsValidationReport:
        ...
        if report.is_clean:
            logger.info("Configuration validated: no placeholders, no missing keys.")
        return report
