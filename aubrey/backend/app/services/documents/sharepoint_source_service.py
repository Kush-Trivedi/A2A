...
class SharePointSourceService:
    ...
    async def ingest(
        ...
        *,
        ...
        if connection.source_type != "sharepoint":
            raise ValidationError(
                f"Connection '{connection.connection_key}' is "
                f"'{connection.source_type}', not SharePoint.",
            )
        location = connection.config

        files = await asyncio.to_thread(self._list_files, location, folder_path)
        if file_name and file_name.strip():
            wanted = file_name.strip().lower()
            files = [f for f in files if f.name.rsplit("/", 1)[-1].lower() == wanted]
            if not files:
                raise ValidationError(
                    f"File '{file_name}' was not found in the connection's site/drive/folder.",
        ...
        return await self._pipeline.run(
            ...
            batch_name=(
                f"sharepoint:{connection.connection_key}/{folder_path or ''}".rstrip("/")
            ),
            files=files,
            download=self._downloader(),
            properties={
                "connection_key": connection.connection_key,
                "site_path": location["site_path"],
    ...
    def _list_files(self, location: dict, folder_path: str) -> list[SourceFile]:
        ...
        try:
            ...
            raise ExternalServiceError(
                "SharePoint listing failed — check the connection's "
                "site/drive/folder and the platform app's access to the site.",
